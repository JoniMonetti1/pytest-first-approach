...
def test_can_delete_user(auth_token):
    headers = {
        'Authorization': 'Bearer ' + auth_token,
        'Content-Type': 'application/json'
    }
    ...
    get_user_response = get_user_by_username(username, headers={
        'Authorization': 'Bearer ' + auth_token
    })
    assert get_user_response.status_code == 200
    user_id = get_user_response.json()['id']

    delete_response = delete_user(user_id, headers={
        'Authorization': 'Bearer ' + auth_token
    })
    assert delete_response.status_code == 204

    not_found_response = get_user(user_id, headers={
        'Authorization': 'Bearer ' + auth_token
    })
    assert not_found_response.status_code == 404, "User should be deleted and not found"
...
def delete_user(user_id: int, headers: dict):
    return requests.delete(BASE_URL + f"/user/{user_id}", headers=headers)
